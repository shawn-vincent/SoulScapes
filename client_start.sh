...
if [ ! -d "$CLIENT_DIR" ]; then
  echo "❌ Directory '$CLIENT_DIR' not found. Please ensure you are running this script from the root of your project."
  exit 1
fi

# Change to the client directory.
cd "$CLIENT_DIR" || { echo "❌ Failed to change directory to '$CLIENT_DIR'."; exit 1; }

# Check if package.json exists in the client directory.
if [ ! -f package.json ]; then
  echo "❌ package.json not found in '$CLIENT_DIR'. Please ensure your client is correctly set up."
  exit 1
fi
