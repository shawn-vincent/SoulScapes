/**
 * slogging – A Versatile, Thread‐Safe Logging Library
 *
 * Author: Shawn Vincent <svincent@svincent.com>
 *
 * Overview:
 *  slogging is a single‐file JavaScript logging library that supports multiple backends:
 *    • Console logging (browser console/stdout) with an independent log level (supports "none")
 *    • File logging with rollover (Node.js only), using size‐based rotation. When the log file
 *       exceeds a given size, it is renamed by appending a date‐formatted string.
 *    • Socket logging (via HTTP POST) to a configured server endpoint with optional buffering.
 *
 ...
 *  default emoji is ⚠️; for serror()/cerror() it is ❌; for sdebug()/csdebug() it is 🐛 (or
 *  “🔧” for internal slogging messages); and for slog()/cslog() it is ℹ️ (or “🔧” for internal messages).
 ...
 *
 *  (Except that years outside 2000–2099 are printed in full.)
 *
 *  Log messages support a ${name} syntax. If the first parameter is an object (a substitution map)
 *  and the second parameter is a string, the string’s `${...}` tokens are replaced using that map.
 *
 ...
 *           // The rolled file’s name is generated by appending a date string to the base file name.
...
(function (global, factory) {
    ...
    // Format: [Mon 1 '25 15:35:26.235 EST] for years 2000–2099; full year otherwise.
    ...
    function getDefaultEmoji(level, component) {
        if (level === "debug") {
            return (component === "slogger") ? "🔧 " : "🐛 ";
        } else if (level === "log") {
            return (component === "slogger") ? "🔧 " : "ℹ️ ";
        } else if (level === "warn") {
            return "⚠️ ";
        } else if (level === "error") {
            return "❌ ";
    ...
    function ensureInitialized() {
        if (!_initialized) {
            ...
            logMessage("log", "🚀 slogging library initialized", [], "slogger", { prependNewlines: 5, skipFile: true, skipSocket: true });
        }
    }
